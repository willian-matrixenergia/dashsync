...
### Fase 5: Refatoração e Funcionalidades da Tela de Login
*   **Alvo:** `app/login/page.tsx` e `src/lib/auth.ts`.
*   **Ação:**
    *   Atualizar regras de Auth (`src/lib/auth.ts`): adicionar restrição para o domínio `@matrixenergia.com` no callback `signIn`, e configurar `CredentialsProvider`.
    *   Reformular o layout de `app/login/page.tsx` introduzindo os campos de e-mail e senha.
    *   Adicionar botão "Solicitar Acesso" linkando para suporte.
    *   Validar estilo seguindo a documentação local da Matrix.
