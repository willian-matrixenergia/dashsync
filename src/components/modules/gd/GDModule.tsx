...
import { GDKpiTable } from "./GDKpiTable";
import { Skeleton } from "@/components/ui/skeleton";
import { Card, CardContent } from "@/components/ui/card";
import { GDKpiRow } from "@/src/types/energy";

export function GDModule() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        if (!res.ok) {
          throw new Error('Erro ao carregar dados de Geração Distribuída.');
        }
  ...
  const headerTitle = `Semana ${getWeekStr(currentWeekDate)} vs. Semana ${getWeekStr(previousWeekDate)}`;
